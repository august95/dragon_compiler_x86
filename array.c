...
void array_brackets_add(struct array_brackets* bracekts, struct node* bracket_node)
{
    ...
    vector_push(bracekts->n_brackets, &bracket_node);
}
...
{
    #warning "Not yet implemented calculation size of array brackets"
    return 0;
}

size_t array_brackets_calculate_size(struct datatype* dtype, struct array_brackets* brackets)
{
    return array_brackets_calculat_size_from_index(dtype, brackets, 0);
}
