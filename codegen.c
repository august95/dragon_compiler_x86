...
void codegen_generate_add_no_compile_time_stack_frame_restore(size_t stack_size)
{
    if(stack_size != 0)
    {
...
void codegen_register_exit_point(int exit_point_id)
{
    ...
    vector_push(gen->codegen_exit_points, &exit_point);
}

struct codegen_exit_point *codegen_current_exit_point()
{
    struct code_generator *gen = current_process->generator;
    return vector_back_ptr_or_null(gen->codegen_exit_points);
}
...
void codegen_end_exit_point()
{
    ...
    free(exit_point);
    vector_pop(gen->codegen_exit_points);
}
...
void codegen_register_entry_point(int entry_point_id)
{
    ...
    vector_push(gen->codegen_entry_points, &entry_point);
}

struct codegen_entry_point *codegen_current_entry_point()
{
    struct code_generator *gen = current_process->generator;
    return vector_back_ptr_or_null(gen->codegen_entry_points);
}
...
void codegen_end_entry_point()
{
    ...
    assert(entry_point);
    free(entry_point);
    vector_pop(gen->codegen_entry_points);
}
...
static const char *asm_keyword_for_size(size_t size, char *tmp_buf)
{
    ...
    default:
        sprintf(tmp_buf, "times %ld db ", (unsigned long)size);
        return tmp_buf;
    }
...
void codegen_generate_entity_access_start(struct resolver_result *result, struct resolver_entity *root_assignment_entity, struct history *history)
{
    ...
    else if (result->flags & RESOLVER_RESULT_FLAG_FIST_ENITITY_PUSH_VALUE)
    {
    ...
    else if (result->flags & RESOLVER_RESULT_FLAG_FIST_ENTITY_LOAD_TO_EBX)
    {
        if (root_assignment_entity->next && root_assignment_entity->next->flag & RESOLVER_ENTITY_FLAG_IS_POINTER_ARRAY_ENTIRY)
        {
...
void codegen_generate_entity_access_for_variable_or_general(struct resolver_result *restult, struct resolver_entity *entity)
{
...
int codegen_entity_rules(struct resolver_entity* last_entity, struct history* history)
{
    ...
    if (datatype_is_struct_or_union_none_pointer(&last_entity->dtype))
    {
...
void codegen_generate_assignment_part(struct node *node, const char *op, struct history *history)
{
    ...
    if (!next_entity)
    {
        if (datatype_is_struct_or_union_none_pointer(&result->last_entity->dtype))
        {
...
bool codegen_resolve_node_for_value(struct node *node, struct history *history)
{
    ...
    if (datatype_is_struct_or_union_none_pointer(&dtype))
    {
    ...
    else if(!(dtype.flags & DATATYPE_FLAG_IS_POINTER))
    {
        ...
        {
            asm_push("mov eax, [eax]"); //move the actual data pointed to by eax, into eax
        }
...
int codegen_set_flag_for_operator(const char *op)
{
    ...
    {
        flag |= EXPRESSION_IS_MULTPILICATION;
    }
    else if (S_EQ(op, "/"))
    {
    ...
    {
        flag |= EXPRESSION_BITSHIFT_LEFT;
    }
    else if (S_EQ(op, ">>"))
    {
        flag |= EXPRESSION_BITSHIFT_RIGHT;
    }
    else if (S_EQ(op, "&"))
    {
...
void codegen_gen_math_for_value(const char *reg, const char *value, int flags, bool is_signed)
{
    ...
    else if (flags & EXPRESSION_IS_MULTPILICATION)
    {
    ...
    else if (flags & EXPRESSION_BITSHIFT_LEFT)
    {
        value = codegen_sub_register(value, DATA_SIZE_BYTE);
        asm_push("sal %s, %s", reg, value);
    }
    else if (flags & EXPRESSION_BITSHIFT_RIGHT)
    {
        value = codegen_sub_register(value, DATA_SIZE_BYTE);
...
void codegen_generate_statement_return(struct node *node)
{
    ...
    codegen_generate_add_no_compile_time_stack_frame_restore(C_ALIGN(function_node_stack_size(node->binded.function)));
...
void codegen_generate_else_or_else_if(struct node* node, int end_label_id)
{
    ...
    {
        compiler_error(current_process, "Unexpected compiler bug");
    }
}
...
void codegen_generate_do_while_stmt(struct node* node)
{
    ...
    asm_push_ins_pop("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value");
    asm_push("cmp eax, 0");
    asm_push("jmp .do_while_start%i", do_while_start_id);
    codegen_end_entry_exit_point();
}
