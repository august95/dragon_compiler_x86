...
struct resolver_scope* resolver_new_scope(struct resolver_process* resolver, void* private, int flags)
{
    ...
    resolver->scope.curret->next = scope;
    scope->prev = resolver->scope.curret;
    resolver->scope.curret = scope;
...
{
    struct resolver_scope* scope = resolver->scope.curret;
    resolver->scope.curret = scope->prev;
...
struct resolver_process* resolver_new_process(struct compile_process* compiler, struct resolver_callbacks* callbacks )
{
    ...
    process->scope.curret = process->scope.root;
    return process;
}
...
struct resolver_entity* resolver_create_new_entity_for_unsupported_node(struct resolver_result* result, struct node* node )
{
    ...
    entity->node;
    entity->flag = RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_LEFT_ENTITY | RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTTY;
    return entity;
}

struct resolver_entity* resolver_create_new_entity_fror_array_bracket(struct resolver_result* result, struct resolver_process* process, struct node* node, struct node* array_index_node, int index, struct datatype* dtype, void* private, struct resolver_scope* scope )
{
...
struct resolver_entity* resolver_create_new_unknown_entity(struct resolver_result* result, struct resolver_process* process,  struct datatype* dtype, struct node* node, struct resolver_scope* scope, int offset)
{
...
struct resolver_entity* resolver_create_new_unary_get_address_entity(struct resolver_result* result, struct resolver_process* process,  struct datatype* dtype, struct node* node, struct resolver_scope* scope, int offset )
{
...
struct resolver_entity* reslover_create_new_entity_for_var_node( struct resolver_process* process, struct node* var_node, void* private, int offset ) 
{
...
struct resolver_entity* reslover_new_entity_for_var_node_no_push( struct resolver_process* process, struct node* var_node, void* private,  int offset, struct resolver_scope* scope )
{
    ...
    {
        entity->flag |= RESOLVER_ENTITY_FLAG_IS_STACK;
    }
...
{
    struct resolver_entity* entity = reslover_new_entity_for_var_node_no_push(process, var_node, private, offset, resolver_process_scope_current(process));
    if(!entity)
    {
        return NULL;
    }

    vector_push(process->scope.curret->entities, &entity);
    return entity;
}
...
struct resolver_entity* resolver_make_entity(struct resolver_process* process, struct resolver_result* result, struct datatype* custom_dtype, struct node* node, struct resolver_entity* guided_entity, struct resolver_scope* scope )
{
    ...
    int flags = guided_entity->flag;
    switch (node->type)
    {
    case NODE_TYPE_VARIABLE:
        entity = reslover_new_entity_for_var_node_no_push(process, node, NULL, offset, scope);
        break;
...
struct resolver_entity* resolver_create_new_entity_for_function(struct resolver_result* result, struct resolver_process* process, struct resolver_entity* left_operand_entity, void* private)
{
...
{
    //we are accessing a structure or union
...
struct resolver_entity* resolver_get_function_in_scope(struct resolver_result* result, struct resolver_process* resolver, const char* func_name, struct resolver_scope* socpe)
{
...
struct resolver_entity* resolver_follow_struct_expression(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    {
        //a->b (do not merge)
        rule.left.flags = RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTTY;
        if(left_entity->type != RESOLVER_ENTITY_TYPE_FUNCTION_CALL)
        {
            //indirection, offset can not be calculated at compile time
            //int* a; 
            //*a = 50
...
{
    if(!(dtype->flags & DATATYPE_FLAG_IS_ARRAY) || array_bracket_count(dtype) <= index)
    {
        bracket_entity->flag = RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_LEFT_ENTITY | RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTTY | RESOLVER_ENTITY_FLAG_IS_POINTER_ARRAY_ENTIRY;
    }
    else if(bracket_node->bracket.inner->type != NODE_TYPE_NUMBER)
    {
        bracket_entity->flag = RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_LEFT_ENTITY | RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTTY;
    }
    else
    {
        bracket_entity->flag = RESOLVER_ENTITY_FLAG_JUST_USE_OFFSET;
    }
...
struct resolver_entity* resolver_follow_array_bracket(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    {
        dtype.array.size = array_brackets_calculat_size_from_index(&dtype, dtype.array.brackets, index +1);
    }
    //we need to reduce the dtype
    void* private = resolver->callbacks.new_array_entity(result, node);
    struct resolver_entity* array_bracket_entity = resolver_create_new_entity_fror_array_bracket(result, resolver, node, node->bracket.inner, index, &dtype, private, scope);
    struct resolver_entity_rule  rule = {};
    resolver_array_bracket_set_flags(array_bracket_entity, &dtype, node, index);
    last_entity->flag |= RESOLVER_ENTITY_FLAG_USES_ARRAY_BRACKETS;
    if(array_bracket_entity->flag & RESOLVER_ENTITY_FLAG_IS_POINTER_ARRAY_ENTIRY)
    {
...
struct resolver_entity* resolver_follow_exp_parantheses(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
...
struct resolver_entity* resolver_follow_cast(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    operand_entity->flag |= RESOLVER_ENTITY_FLAG_WAS_CASTED;
...
struct resolver_entity* resolver_follow_indirection(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    struct resolver_entity* unary_indirection_entity = resolver_create_new_unary_indirection_entity(result,resolver, node, node->unary.indirection.depth);
...
struct resolver_entity* resolver_follow_unary_address(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    struct resolver_entity* last_entity = resolver_result_peek(result);
    struct resolver_entity* unary_address_entity = resolver_create_new_unary_get_address_entity(result,resolver,&last_entity->dtype, node,last_entity->scope, last_entity->offset);
...
struct resolver_entity* resolver_follow_part_return_entity(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    switch(node->type)
    {
        ...
        case NODE_TYPE_EXPRESSION_PARANTHESES:
            entity = resolver_follow_exp_parantheses(resolver, node, result);
            break;
        case NODE_TYPE_CAST:
        ...
        default:
        {
            //requiers more processing later on
...
void resolver_rule_apply_rules(struct resolver_entity* rule_entity, struct resolver_entity* left_entity, struct resolver_entity* right_entity)
{
    ...
    if(left_entity)
    {
        left_entity->flag |= rule_entity->rule.left.flags;
    }
    if(right_entity)
    {
        right_entity->flag |= rule_entity->rule.right.flags;
    }
}
...
struct resolver_entity* resolver_merge_compile_times_result(struct resolver_process* resolver, struct resolver_result* result, struct resolver_entity* left_entity, struct resolver_entity* right_entity)
{
    ...
    {
        if(left_entity->flag & RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTTY ||
...
{
    int flags = RESOLVER_RESULT_FLAG_FIST_ENITITY_PUSH_VALUE;
    ...
    while(entity)
    {
        ...
        {
            flags |= RESOLVER_RESULT_FLAG_FIST_ENTITY_LOAD_TO_EBX | RESOLVER_RESULT_FLAG_DOES_GET_ADDRESS;
        ...
        if(entity->type == RESOLVER_ENTITY_TYPE_ARRAY_BRACKET)
        {
            ...
            if(entity->flag & RESOLVER_ENTITY_FLAG_IS_POINTER_ARRAY_ENTIRY)
            {
